from flask import Flask, jsonify, send_from_directory, request
import mysql.connector
from mysql.connector import Error
from werkzeug.security import generate_password_hash, check_password_hash
import os
import click
# IMPORTAÇÃO ALTERADA: Importa os templates methods
from db_utils import create_db_connection, execute_db_query, execute_db_transaction
from routes.alunos_bp import alunos_bp
from routes.forum_bp import forum_bp

# CORREÇÃO AQUI: static_folder deve apontar para o nome real da sua pasta de frontend
app = Flask(__name__, static_folder='../PROJETO_FINAL', static_url_path='/')
app.register_blueprint(alunos_bp)
app.register_blueprint(forum_bp)


# ADIÇÃO: Configuração para uploads de materiais
UPLOAD_FOLDER = os.path.join(app.static_folder, 'uploads')
if not os.path.exists(UPLOAD_FOLDER):
    os.makedirs(UPLOAD_FOLDER)
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER


# Rota para servir a página HTML principal (index.html)
@app.route('/')
def serve_index():
    return send_from_directory(app.static_folder, 'index.html')

# Rota para servir todas as outras páginas HTML na pasta frontend
@app.route('/<path:filename>')
def serve_static_files(filename):
    return send_from_directory(app.static_folder, filename)

# ====================================================================================================
# ROTAS DE ADMINISTRAÇÃO (Template Method)
# ====================================================================================================

"""def _get_dashboard_stats_logic(connection, cursor):
    # Contar alunos
    cursor.execute("SELECT COUNT(*) as total_alunos FROM alunos")
    total_alunos = cursor.fetchone()['total_alunos']
    
    # Contar aulas ministradas (status 'completed')
    cursor.execute("SELECT COUNT(*) as aulas_ministradas FROM classes WHERE status = 'completed'")
    aulas_ministradas = cursor.fetchone()['aulas_ministradas']
    
    # Calcular frequência média
    cursor.execute("SELECT COUNT(*) as total_presentes FROM attendance_records WHERE attendance_status = 'P'")
    total_presentes = cursor.fetchone()['total_presentes']
    
    cursor.execute("SELECT COUNT(*) as total_registros FROM attendance_records")
    total_registros = cursor.fetchone()['total_registros']
    
    frequencia_media = (total_presentes / total_registros * 100) if total_registros > 0 else 0
    
    return {
        'success': True,
        'total_alunos': total_alunos,
        'aulas_ministradas': aulas_ministradas,
        'frequencia_media': round(frequencia_media)
    }, 200

@app.route('/dashboard/stats', methods=['GET'])
def get_dashboard_stats():
    return execute_db_query(_get_dashboard_stats_logic, error_message='Erro ao buscar estatísticas')

def _get_users_logic(connection, cursor):
    cursor.execute("SELECT id, username, full_name, role, student_id, last_login, total_logins, online_status, data_criacao FROM users")
    users = cursor.fetchall()
    for user in users:
        if user.get('last_login'):
            user['last_login'] = user['last_login'].isoformat()
        if user.get('data_criacao'):
            user['data_criacao'] = user['data_criacao'].isoformat()
            
    return users, 200

@app.route('/users', methods=['GET'])
def get_users():
    return execute_db_query(_get_users_logic, error_message='Erro ao buscar usuários')

def _get_user_by_id_logic(connection, cursor, user_id):
    query = "SELECT id, username, full_name, role, student_id, last_login, total_logins, online_status FROM users WHERE id = %s"
    cursor.execute(query, (user_id,))
    user = cursor.fetchone()"""
    
    if user:
        if user.get('last_login'):
            user['last_login'] = user['last_login'].isoformat()
        return user, 200
    else:
        return {'message': 'Usuário não encontrado!'}, 404

@app.route('/users/<int:user_id>', methods=['GET'])
def get_user_by_id(user_id):
    return execute_db_query(_get_user_by_id_logic, user_id=user_id, error_message='Erro ao buscar usuário por ID')


def _add_user_logic(connection, cursor, user_data):
    username = user_data.get('username')
    password = user_data.get('password')
    full_name = user_data.get('full_name')
    role = user_data.get('role')
    student_id_raw = user_data.get('student_id')

    if not username or not password or not role:
        raise ValueError('Username, Password e Role são obrigatórios!')

    allowed_roles = ['student', 'teacher']
    if role not in allowed_roles:
        raise ValueError('Role inválida. Opções válidas: student, teacher.')

    student_id = None
    if role == 'student':
        if student_id_raw:
            try:
                student_id = int(student_id_raw)
            except ValueError:
                raise ValueError('Para o perfil de Aluno, o ID de Aluno deve ser um número válido.')

    hashed_password = generate_password_hash(password)

    query = """
    INSERT INTO users (username, password_hash, full_name, role, student_id)
    VALUES (%s, %s, %s, %s, %s)
    """
    values = (username, hashed_password, full_name, role, student_id)
    cursor.execute(query, values)
    
    return {'success': True, 'message': 'Usuário adicionado com sucesso!'}, 201, None

@app.route('/users/add', methods=['POST'])
def add_user():
    user_data = request.get_json()
    return execute_db_transaction(_add_user_logic, user_data=user_data, rollback_message='Erro ao adicionar usuário')
        

def _edit_user_logic(connection, cursor, user_id, user_data):
    username = user_data.get('username')
    set_clauses = []
    values = []
    
    if 'username' in user_data:
        set_clauses.append("username = %s")
        values.append(user_data['username'])
    if 'full_name' in user_data:
        set_clauses.append("full_name = %s")
        values.append(user_data['full_name'])
    if 'role' in user_data:
        allowed_roles = ['student', 'teacher']
        if user_data.get('role') not in allowed_roles:
            raise ValueError('Role inválida. Opções válidas: student, teacher.')
        set_clauses.append("role = %s")
        values.append(user_data['role'])
    if 'student_id' in user_data:
        edit_student_id_raw = user_data.get('student_id')
        edit_student_id = None
        if user_data.get('role', '') == 'student':
            if edit_student_id_raw:
                try:
                    edit_student_id = int(edit_student_id_raw)
                except ValueError:
                    raise ValueError('Para o perfil de Aluno, o ID de Aluno deve ser um número válido.')
        set_clauses.append("student_id = %s")
        values.append(edit_student_id)
    if 'last_login' in user_data:
        set_clauses.append("last_login = %s")
        values.append(user_data['last_login'])
    if 'total_logins' in user_data:
        set_clauses.append("total_logins = %s")
        values.append(user_data['total_logins'])
    if 'online_status' in user_data:
        set_clauses.append("online_status = %s")
        values.append(user_data['online_status'])
    if 'password' in user_data and user_data['password']:
        set_clauses.append("password_hash = %s")
        values.append(generate_password_hash(user_data['password']))

    if not set_clauses:
        return {'success': False, 'message': 'Nenhum dado para atualizar.'}, 400, None

    query = f"UPDATE users SET {', '.join(set_clauses)} WHERE id = %s"
    values.append(user_id)
    
    cursor.execute(query, tuple(values))
    
    if cursor.rowcount > 0:
        return {'success': True, 'message': 'Usuário atualizado com sucesso!'}, 200, None
    else:
        return {'success': False, 'message': 'Usuário não encontrado para atualização.'}, 404, None

@app.route('/users/edit/<int:user_id>', methods=['PUT'])
def edit_user(user_id):
    user_data = request.get_json()
    return execute_db_transaction(_edit_user_logic, user_id=user_id, user_data=user_data, rollback_message='Erro ao atualizar usuário')


def _delete_user_logic(connection, cursor, user_id):
    query = "DELETE FROM users WHERE id = %s"
    cursor.execute(query, (user_id,))
    
    if cursor.rowcount > 0:
        return {'success': True, 'message': 'Usuário excluído com sucesso!'}, 200, None
    else:
        return {'success': False, 'message': 'Usuário não encontrado.'}, 404, None

@app.route('/users/delete/<int:user_id>', methods=['DELETE'])
def delete_user(user_id):
    return execute_db_transaction(_delete_user_logic, user_id=user_id, rollback_message='Erro ao deletar usuário')


# ====================================================================================================
# ROTAS DE AUTENTICAÇÃO
# ====================================================================================================
# NOTA: O Login é uma exceção e não usa o Template Method de escrita pois o commit/rollback
# de logout/login é aninhado e a lógica precisa checar o resultado do SELECT antes de fazer o UPDATE.
@app.route('/login', methods=['POST'])
def login():
    credentials = request.get_json()
    username = credentials.get('username')
    password = credentials.get('password')

    if not username or not password:
        return jsonify({'success': False, 'message': 'Nome de usuário e senha são obrigatórios.'}), 400
    
    connection = create_db_connection()
    if not connection:
        return jsonify({'success': False, 'message': 'MAINTENANCE_MODE'}), 503

    try:
        cursor = connection.cursor(dictionary=True)
        
        query = "SELECT id, username, password_hash, full_name, role, student_id, must_change_password FROM users WHERE username = %s"
        cursor.execute(query, (username,))
        user = cursor.fetchone()

        if user and check_password_hash(user['password_hash'], password):
            cursor_update = connection.cursor()
            update_query = "UPDATE users SET last_login = NOW(), total_logins = total_logins + 1, online_status = 'Online' WHERE id = %s"
            cursor_update.execute(update_query, (user['id'],))
            connection.commit()
            cursor_update.close()

            response_data = {
                'success': True,
                'message': 'Login bem-sucedido!',
                'user': {
                    'id': user['id'],
                    'username': user['username'],
                    'full_name': user['full_name'],
                    'role': user['role'],
                    'student_id': user['student_id'],
                    'must_change_password': user['must_change_password']
                }
            }
            return jsonify(response_data), 200
        else:
            return jsonify({'success': False, 'message': 'Nome de usuário ou senha incorretos.'}), 401
    except Error as e:
        # print(f"Erro no login: {e}")
        return jsonify({'success': False, 'message': 'Erro interno do servidor.'}), 500
    finally:
        if connection and connection.is_connected():
            cursor.close()
            connection.close()

def _logout_logic(connection, cursor, user_id):
    query = "UPDATE users SET online_status = 'Offline' WHERE id = %s"
    cursor.execute(query, (user_id,))
    
    return {'success': True, 'message': 'Logout bem-sucedido!'}, 200, None

@app.route('/logout/<int:user_id>', methods=['POST'])
def logout(user_id):
    return execute_db_transaction(_logout_logic, user_id=user_id, rollback_message='Erro no logout')

# ====================================================================================================
# ROTAS PARA CLASSES
# ====================================================================================================
def _get_classes_logic(connection, cursor):
    cursor.execute("SELECT id, title, date, status, description FROM classes ORDER BY date ASC")
    classes = cursor.fetchall()
    for cls in classes:
        if cls.get('date'):
            cls['date'] = cls['date'].isoformat()
    return classes, 200

@app.route('/classes', methods=['GET'])
def get_classes():
    return execute_db_query(_get_classes_logic, error_message='Erro ao buscar classes')

def _get_class_by_id_logic(connection, cursor, class_id):
    query = "SELECT id, title, date, status, description FROM classes WHERE id = %s"
    cursor.execute(query, (class_id,))
    class_item = cursor.fetchone()
    
    if class_item:
        if class_item.get('date'):
            class_item['date'] = class_item['date'].isoformat()
        return class_item, 200
    else:
        return {'message': 'Aula não encontrada!'}, 404

@app.route('/classes/<int:class_id>', methods=['GET'])
def get_class_by_id(class_id):
    return execute_db_query(_get_class_by_id_logic, class_id=class_id, error_message='Erro ao buscar aula por ID')

def _add_class_logic(connection, cursor, class_data):
    title = class_data.get('title')
    date = class_data.get('date')
    status = class_data.get('status', 'future')
    description = class_data.get('description')

    if not title or not date:
        raise ValueError('Título e Data são obrigatórios.')

    query = "INSERT INTO classes (title, date, status, description) VALUES (%s, %s, %s, %s)"
    values = (title, date, status, description)
    cursor.execute(query, values)
    
    return {'success': True, 'message': 'Aula adicionada com sucesso!'}, 201, None

@app.route('/classes/add', methods=['POST'])
def add_class():
    class_data = request.get_json()
    return execute_db_transaction(_add_class_logic, class_data=class_data, rollback_message='Erro ao adicionar aula')

def _edit_class_logic(connection, cursor, class_id, class_data):
    set_clauses = []
    values = []

    if 'title' in class_data:
        set_clauses.append("title = %s")
        values.append(class_data['title'])
    if 'date' in class_data:
        set_clauses.append("date = %s")
        values.append(class_data['date'])
    if 'status' in class_data:
        set_clauses.append("status = %s")
        values.append(class_data['status'])
    if 'description' in class_data:
        set_clauses.append("description = %s")
        values.append(class_data['description'])
    
    if not set_clauses:
        return {'success': False, 'message': 'Nenhum dado para atualizar.'}, 400, None

    query = f"UPDATE classes SET {', '.join(set_clauses)} WHERE id = %s"
    values.append(class_id)
    
    cursor.execute(query, tuple(values))
    
    if cursor.rowcount > 0:
        return {'success': True, 'message': 'Aula atualizada com sucesso!'}, 200, None
    else:
        return {'success': False, 'message': 'Aula não encontrada.'}, 404, None

@app.route('/classes/edit/<int:class_id>', methods=['PUT'])
def edit_class(class_id):
    class_data = request.get_json()
    return execute_db_transaction(_edit_class_logic, class_id=class_id, class_data=class_data, rollback_message='Erro ao atualizar aula')

def _delete_class_logic(connection, cursor, class_id):
    query = "DELETE FROM classes WHERE id = %s"
    cursor.execute(query, (class_id,))
    
    if cursor.rowcount > 0:
        return {'success': True, 'message': 'Aula excluída com sucesso!'}, 200, None
    else:
        return {'success': False, 'message': 'Aula não encontrada.'}, 404, None

@app.route('/classes/delete/<int:class_id>', methods=['DELETE'])
def delete_class(class_id):
    return execute_db_transaction(_delete_class_logic, class_id=class_id, rollback_message='Erro ao deletar aula')

# ====================================================================================================
# ROTAS PARA REGISTROS DE FREQUÊNCIA (attendance_records)
# ====================================================================================================
def _get_attendance_records_logic(connection, cursor):
    query = """
    SELECT ar.id, ar.student_id, a.nome as student_name, ar.class_id, c.title as class_title, ar.attendance_status, ar.recorded_at
    FROM attendance_records ar
    JOIN alunos a ON ar.student_id = a.id
    JOIN classes c ON ar.class_id = c.id
    ORDER BY ar.recorded_at DESC
    """
    cursor.execute(query)
    records = cursor.fetchall()
    for rec in records:
        if rec.get('recorded_at'):
            rec['recorded_at'] = rec['recorded_at'].isoformat()
    return records, 200

@app.route('/attendance', methods=['GET'])
def get_attendance_records():
    return execute_db_query(_get_attendance_records_logic, error_message='Erro ao buscar registros de frequência')

def _get_attendance_by_student_logic(connection, cursor, student_id):
    query = """
    SELECT ar.id, ar.student_id, ar.class_id, c.title as class_title, c.date as class_date, ar.attendance_status, ar.recorded_at
    FROM attendance_records ar
    JOIN classes c ON ar.class_id = c.id
    WHERE ar.student_id = %s
    ORDER BY c.date ASC
    """
    cursor.execute(query, (student_id,))
    records = cursor.fetchall()
    for rec in records:
        if rec.get('recorded_at'):
            rec['recorded_at'] = rec['recorded_at'].isoformat()
        if rec.get('class_date'):
            rec['class_date'] = rec['class_date'].isoformat()
    return records, 200

@app.route('/attendance/student/<int:student_id>', methods=['GET'])
def get_attendance_by_student(student_id):
    return execute_db_query(_get_attendance_by_student_logic, student_id=student_id, error_message='Erro ao buscar frequência do aluno')


def _update_status_alunos_logic(cursor, student_id, connection=None):
    # Lógica de atualização de status (faltas)
    count_absences_query = """
        SELECT COUNT(*) FROM attendance_records
        WHERE student_id = %s AND attendance_status IN ('F', 'Fj')
    """
    cursor.execute(count_absences_query, (student_id,))
    total_absences = cursor.fetchone()[0]

    update_status_query = """
        INSERT INTO status_alunos (id, faltas, situacao)
        VALUES (%s, %s, %s)
        ON DUPLICATE KEY UPDATE faltas = VALUES(faltas), situacao = VALUES(situacao)
    """
    cursor.execute(update_status_query, (student_id, total_absences, 'Ativo'))

def _add_attendance_record_logic(connection, cursor, record_data):
    student_id = record_data.get('student_id')
    class_id = record_data.get('class_id')
    attendance_status = record_data.get('attendance_status')

    if not student_id or not class_id or not attendance_status:
        raise ValueError('Student ID, Class ID e Status de Frequência são obrigatórios.')
    
    # 1. Checa se o registro existe
    check_query = "SELECT id FROM attendance_records WHERE student_id = %s AND class_id = %s"
    cursor.execute(check_query, (student_id, class_id))
    existing_record = cursor.fetchone()

    if existing_record:
        record_id = existing_record[0]
        update_query = "UPDATE attendance_records SET attendance_status = %s WHERE id = %s"
        cursor.execute(update_query, (attendance_status, record_id))
    else:
        insert_query = "INSERT INTO attendance_records (student_id, class_id, attendance_status) VALUES (%s, %s, %s)"
        cursor.execute(insert_query, (student_id, class_id, attendance_status))
        record_id = cursor.lastrowid
        
    # 2. Atualiza o status consolidado do aluno (faltas)
    _update_status_alunos_logic(cursor, student_id)

    return {'success': True, 'message': 'Registro de frequência salvo com sucesso!', 'id': record_id}, 200, None

@app.route('/attendance/add', methods=['POST'])
def add_attendance_record():
    record_data = request.get_json()
    return execute_db_transaction(_add_attendance_record_logic, record_data=record_data, rollback_message='Erro ao adicionar/atualizar registro de frequência e faltas')


def _edit_attendance_record_logic(connection, cursor, record_id, record_data):
    attendance_status = record_data.get('attendance_status')

    if not attendance_status:
        raise ValueError('Status de Frequência é obrigatório.')

    query = "UPDATE attendance_records SET attendance_status = %s WHERE id = %s"
    values = (attendance_status, record_id)
    cursor.execute(query, values)
    
    if cursor.rowcount > 0:
        get_student_id_query = "SELECT student_id FROM attendance_records WHERE id = %s"
        cursor.execute(get_student_id_query, (record_id,))
        result = cursor.fetchone()
        
        if result:
            student_id = result[0]
            # Atualiza o status consolidado do aluno (faltas)
            _update_status_alunos_logic(cursor, student_id)

        return {'success': True, 'message': 'Registro de frequência atualizado com sucesso!'}, 200, None
    else:
        return {'success': False, 'message': 'Registro de frequência não encontrado.'}, 404, None

@app.route('/attendance/edit/<int:record_id>', methods=['PUT'])
def edit_attendance_record(record_id):
    record_data = request.get_json()
    return execute_db_transaction(_edit_attendance_record_logic, record_id=record_id, record_data=record_data, rollback_message='Erro ao atualizar registro de frequência')

def _delete_attendance_record_logic(connection, cursor, record_id):
    # 1. Obter o student_id antes de deletar
    get_student_id_query = "SELECT student_id FROM attendance_records WHERE id = %s"
    cursor.execute(get_student_id_query, (record_id,))
    result = cursor.fetchone()
    student_id = None
    if result:
        student_id = result[0]

    # 2. Deletar o registro
    query = "DELETE FROM attendance_records WHERE id = %s"
    cursor.execute(query, (record_id,))
    
    if cursor.rowcount > 0:
        if student_id:
            # 3. Atualizar o status consolidado do aluno (faltas)
            _update_status_alunos_logic(cursor, student_id)

        return {'success': True, 'message': 'Registro de frequência excluído com sucesso!'}, 200, None
    else:
        return {'success': False, 'message': 'Registro de frequência não encontrado.'}, 404, None

@app.route('/attendance/delete/<int:record_id>', methods=['DELETE'])
def delete_attendance_record(record_id):
    return execute_db_transaction(_delete_attendance_record_logic, record_id=record_id, rollback_message='Erro ao deletar registro de frequência')

# ====================================================================================================
# ROTAS PARA STATUS DOS ALUNOS (status_alunos - AQUI SERÁ A TABELA CONSOLIDADA)
# ====================================================================================================
def _get_status_alunos_logic(connection, cursor):
    query = """
    SELECT sa.id, a.nome as student_name, sa.faltas, sa.situacao
    FROM status_alunos sa
    JOIN alunos a ON sa.id = a.id
    ORDER BY a.nome ASC
    """
    cursor.execute(query)
    statuses = cursor.fetchall()
    return statuses, 200

@app.route('/status_alunos', methods=['GET'])
def get_status_alunos():
    return execute_db_query(_get_status_alunos_logic, error_message='Erro ao buscar status dos alunos')

# ADIÇÃO: Rotas para Atividades dos Alunos
def _get_atividades_alunos_logic(connection, cursor):
    query = """
    SELECT aa.*, a.nome as student_name
    FROM atividades_alunos aa
    JOIN alunos a ON aa.id = a.id
    ORDER BY a.nome ASC
    """
    cursor.execute(query)
    activities = cursor.fetchall()
    return activities, 200

@app.route('/atividades_alunos', methods=['GET'])
def get_atividades_alunos():
    return execute_db_query(_get_atividades_alunos_logic, error_message='Erro ao buscar atividades dos alunos')


def _update_aula_status_logic(connection, cursor, aluno_id, update_data):
    aula_col = update_data.get('aula_col')
    new_status = update_data.get('new_status')

    if not aula_col or not new_status or not aluno_id:
        raise ValueError('Dados de atualização insuficientes.')
    
    if aula_col not in [f'aula_{i}' for i in range(1, 11)]:
        raise ValueError('Coluna de aula inválida.')
    
    # 1. Atualiza a coluna da aula
    query_aula = f"UPDATE atividades_alunos SET {aula_col} = %s WHERE id = %s"
    cursor.execute(query_aula, (new_status, aluno_id))
    
    if cursor.rowcount > 0:
        # 2. Recalcula o total de atividades enviadas/verificadas
        sum_query_parts = [f"CASE WHEN aula_{i} IN ('Enviada', 'Verificada') THEN 1 ELSE 0 END" for i in range(1, 11)]
        sum_query = f"SELECT ({' + '.join(sum_query_parts)}) FROM atividades_alunos WHERE id = %s"
        cursor.execute(sum_query, (aluno_id,))
        calculated_total = cursor.fetchone()[0]
        
        # 3. Atualiza o total_enviadas
        update_total_query = "UPDATE atividades_alunos SET total_enviadas = %s WHERE id = %s"
        cursor.execute(update_total_query, (calculated_total, aluno_id))
        
        return {'success': True, 'message': f'Status da {aula_col} e total atualizados com sucesso!'}, 200, None
    else:
        return {'success': False, 'message': 'Aluno ou aula não encontrada para atualização.'}, 404, None

@app.route('/atividades_alunos/update_aula/<int:aluno_id>', methods=['PUT'])
def update_aula_status(aluno_id):
    update_data = request.get_json()
    return execute_db_transaction(_update_aula_status_logic, aluno_id=aluno_id, update_data=update_data, rollback_message='Erro ao atualizar status da aula')


# ====================================================================================================
# ROTAS PARA MATERIAIS
# ====================================================================================================
def _get_materials_logic(connection, cursor):
    cursor.execute("SELECT id, name, file_type, file_size, upload_date, description, file_path FROM materials ORDER BY upload_date DESC")
    materials = cursor.fetchall()
    for mat in materials:
        if mat.get('upload_date'):
            mat['upload_date'] = mat['upload_date'].isoformat()
    return materials, 200

@app.route('/materials', methods=['GET'])
def get_materials():
    return execute_db_query(_get_materials_logic, error_message='Erro ao buscar materiais')

@app.route('/materials/upload', methods=['POST'])
def upload_material():
    if 'file' not in request.files:
        return jsonify({'success': False, 'message': 'Nenhum arquivo enviado.'}), 400
    
    file = request.files['file']
    if file.filename == '':
        return jsonify({'success': False, 'message': 'Nenhum arquivo selecionado.'}), 400
    
    name = request.form.get('name', file.filename)
    description = request.form.get('description', '')
    
    if not file:
        return jsonify({'success': False, 'message': 'Erro no upload do arquivo.'}), 500

    # Lógica de upload e salvamento do arquivo (fora do Template Method)
    filename = file.filename
    filepath = os.path.join(app.config['UPLOAD_FOLDER'], filename)
    file.save(filepath)

    def _upload_material_logic(connection, cursor, filename, name, description, file):
        # Tenta inserir no BD
        try:
            query = """
            INSERT INTO materials (name, file_type, file_size, description, file_path)
            VALUES (%s, %s, %s, %s, %s)
            """
            values = (name, file.content_type, file.content_length, description, filename)
            cursor.execute(query, values)
            material_id = cursor.lastrowid
            return {'success': True, 'message': 'Material enviado e registrado com sucesso!', 'id': material_id}, 201, None
        except Error as e:
            # Se der erro de BD, o Template fará o rollback.
            # Precisamos limpar o arquivo salvo no sistema de arquivos.
            if os.path.exists(filepath):
                os.remove(filepath)
            # Re-lança o erro para o Template Method (Transaction) capturar
            raise e
        
    try:
        return execute_db_transaction(_upload_material_logic, filename=filename, name=name, description=description, file=file, rollback_message='Erro interno do servidor ao registrar material.')
    except Exception as e:
        # Erro de BD já tratado pelo execute_db_transaction.
        # Aqui, estamos apenas tratando o caso de erro de conexão inicial ou Validações de Flask
        if os.path.exists(filepath):
            os.remove(filepath) # Limpa se falhar a conexão inicial
        return jsonify({'success': False, 'message': 'Erro de conexão com o banco de dados.'}), 500
        

@app.route('/materials/download/<path:filename>', methods=['GET'])
def download_material(filename):
    return send_from_directory(app.config['UPLOAD_FOLDER'], filename, as_attachment=True)


def _edit_material_logic(connection, cursor, material_id, material_data):
    set_clauses = []
    values = []

    if 'name' in material_data:
        set_clauses.append("name = %s")
        values.append(material_data['name'])
    if 'description' in material_data:
        set_clauses.append("description = %s")
        values.append(material_data['description'])
    
    if not set_clauses:
        return {'success': False, 'message': 'Nenhum dado para atualizar.'}, 400, None

    query = f"UPDATE materials SET {', '.join(set_clauses)} WHERE id = %s"
    values.append(material_id)
    
    cursor.execute(query, tuple(values))

    if cursor.rowcount > 0:
        return {'success': True, 'message': 'Material atualizado com sucesso!'}, 200, None
    else:
        return {'success': False, 'message': 'Material não encontrado.'}, 404, None

@app.route('/materials/edit/<int:material_id>', methods=['PUT'])
def edit_material(material_id):
    material_data = request.get_json()
    return execute_db_transaction(_edit_material_logic, material_id=material_id, material_data=material_data, rollback_message='Erro ao atualizar material')


def _delete_material_logic(connection, cursor, material_id):
    # 1. Obter o file_path antes de deletar (requer cursor dictionary=True)
    cursor_dict = connection.cursor(dictionary=True)
    cursor_dict.execute("SELECT file_path FROM materials WHERE id = %s", (material_id,))
    material = cursor_dict.fetchone()
    cursor_dict.close()
    
    if material and material.get('file_path'):
        filepath = os.path.join(app.config['UPLOAD_FOLDER'], material['file_path'])
        if os.path.exists(filepath):
            os.remove(filepath)
        else:
            # print(f"Arquivo {filepath} não encontrado no servidor, mas continuará a exclusão do DB.")
            pass

    # 2. Deletar do BD
    query = "DELETE FROM materials WHERE id = %s"
    cursor.execute(query, (material_id,))

    if cursor.rowcount > 0:
        return {'success': True, 'message': 'Material excluído com sucesso!'}, 200, None
    else:
        return {'success': False, 'message': 'Material não encontrado.'}, 404, None


@app.route('/materials/delete/<int:material_id>', methods=['DELETE'])
def delete_material(material_id):
    return execute_db_transaction(_delete_material_logic, material_id=material_id, rollback_message='Erro ao deletar material')

# ====================================================================================================
# ROTAS PARA AVISOS
# ====================================================================================================
def _get_avisos_logic(connection, cursor):
    query = """
    SELECT a.id, a.titulo, a.mensagem, a.data_criacao, u.full_name as autor
    FROM avisos a
    LEFT JOIN users u ON a.user_id = u.id
    ORDER BY a.data_criacao DESC
    """
    cursor.execute(query)
    avisos = cursor.fetchall()
    for aviso in avisos:
        if aviso.get('data_criacao'):
            aviso['data_criacao'] = aviso['data_criacao'].isoformat()
    return avisos, 200

@app.route('/avisos', methods=['GET'])
def get_avisos():
    return execute_db_query(_get_avisos_logic, error_message='Erro ao buscar avisos')

def _add_aviso_logic(connection, cursor, aviso_data):
    titulo = aviso_data.get('titulo')
    mensagem = aviso_data.get('mensagem')
    user_id = aviso_data.get('user_id')

    if not titulo or not mensagem or not user_id:
        raise ValueError('Título, mensagem e ID do usuário são obrigatórios.')

    query = "INSERT INTO avisos (titulo, mensagem, user_id) VALUES (%s, %s, %s)"
    values = (titulo, mensagem, user_id)
    cursor.execute(query, values)
    
    return {'success': True, 'message': 'Aviso adicionado com sucesso!'}, 201, None

@app.route('/avisos/add', methods=['POST'])
def add_aviso():
    aviso_data = request.get_json()
    return execute_db_transaction(_add_aviso_logic, aviso_data=aviso_data, rollback_message='Erro ao adicionar aviso')

def _delete_aviso_logic(connection, cursor, aviso_id):
    query = "DELETE FROM avisos WHERE id = %s"
    cursor.execute(query, (aviso_id,))
    
    if cursor.rowcount > 0:
        return {'success': True, 'message': 'Aviso excluído com sucesso!'}, 200, None
    else:
        return {'success': False, 'message': 'Aviso não encontrado.'}, 404, None

@app.route('/avisos/delete/<int:aviso_id>', methods=['DELETE'])
def delete_aviso(aviso_id):
    return execute_db_transaction(_delete_aviso_logic, aviso_id=aviso_id, rollback_message='Erro ao deletar aviso')


@app.cli.command("init-db")
def init_db_command():
    """Limpa os dados existentes e cria novas tabelas."""
    setup_database()
    click.echo("Banco de dados inicializado.")


def _force_change_password_logic(connection, cursor, data):
    user_id = data.get('user_id')
    new_password = data.get('new_password')

    if not user_id or not new_password:
        raise ValueError('Dados incompletos.')

    # Gera o hash da nova senha
    new_hashed_password = generate_password_hash(new_password)

    # Atualiza a senha e define must_change_password como FALSE (0)
    query = "UPDATE users SET password_hash = %s, must_change_password = 0 WHERE id = %s"
    cursor.execute(query, (new_hashed_password, user_id))
    
    return {'success': True, 'message': 'Senha alterada com sucesso!'}, 200, None

@app.route('/users/force-change-password', methods=['POST'])
def force_change_password():
    data = request.get_json()
    return execute_db_transaction(_force_change_password_logic, data=data, rollback_message='Erro ao alterar senha')


if __name__ == '__main__':
    app.run(debug=True, port=5000)
